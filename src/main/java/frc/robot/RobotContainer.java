...
public class RobotContainer {
  ...
  // private final LauncherSubsystem m_InverseLauncher = new LauncherSubsystem();

  // The driver's controller
  XboxController m_driverController = new XboxController(OIConstants.kDriverControllerPort);
  // The mechs controller
  ...
  public RobotContainer() {
    ...
    /*
        m_launcher.setDefaultCommand(
            new RunCommand(() -> m_InverseLauncher.stopInverseLauncher(), m_InverseLauncher));
    */
    // m_launcher));
  }
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_mechController, XboxController.Button.kX.value)
        .onTrue(new InstantCommand(() -> m_arm.setTargetPosition(Constants.Arm.kHomePosition)));

    new JoystickButton(m_mechController, XboxController.Button.kA.value)
        .onTrue(new InstantCommand(() -> m_arm.setTargetPosition(Constants.Arm.kIntakePosition)));

    // This is intake position -1.17
    // new Trigger(
    // () ->
    ...
    // released)
    new JoystickButton(m_mechController, XboxController.Button.kRightBumper.value)
        .whileTrue(
            new RunCommand(() -> m_intake.setPower(Constants.Intake.kIntakePower), m_intake));
    // .onFalse(m_intake.retract());

    new JoystickButton(m_mechController, XboxController.Button.kLeftBumper.value)
        .whileTrue(
            new RunCommand(() -> m_intake.setPower(Constants.Intake.kRevIntakePower), m_intake));

    // launcher controls (button to pre-spin the launcher and button to launch)
    // code does not work
    // new JoystickButton(m_mechController, XboxController.Axis.kRightTrigger.value)
    // .whileTrue(new RunCommand(() -> m_launcher.runLauncher(), m_launcher));

    // new JoystickButton(m_mechController, XboxController.Axis.kLeftTrigger.value)
    // .onTrue(m_intake.feedLauncher(m_launcher));

    // launcher controls(button to pre-spin launcher and button to launnch)
    new JoystickButton(m_mechController, XboxController.Button.kY.value)
        .whileTrue(new RunCommand(() -> m_launcher.runLauncher(), m_launcher));
    /*
    new Trigger(//change this to control button
            () ->
                m_mechController.getLeftTriggerAxis()
                    > Constants.OIConstants.kTriggerButtonThreshold)
        .whileTrue(new RunCommand(() -> m_launcher.runLauncher(), m_launcher));
    ...
    new Trigger(
            () ->
                m_mechController.getLeftTriggerAxis()
                    > Constants.OIConstants.kTriggerButtonThreshold)
        .whileTrue(new RunCommand(() -> m_launcher.runInverseLauncher(), m_launcher));
  }
