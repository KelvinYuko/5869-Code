...
public class ArmSubsystem extends SubsystemBase {
  private CANSparkFlex arm_motor;
  ...
  public ArmSubsystem() {
    ...
    arm_motor = new CANSparkFlex(Constants.Arm.kArmCanId, MotorType.kBrushless);
    arm_motor.setInverted(false);
    arm_motor.setSmartCurrentLimit(Constants.Arm.kCurrentLimit);
    arm_motor.setIdleMode(IdleMode.kBrake);
    arm_motor.enableSoftLimit(SoftLimitDirection.kForward, true);
    arm_motor.enableSoftLimit(SoftLimitDirection.kReverse, true);
    arm_motor.setSoftLimit(SoftLimitDirection.kForward, (float) Constants.Arm.kSoftLimitForward);
    arm_motor.setSoftLimit(SoftLimitDirection.kReverse, (float) Constants.Arm.kSoftLimitReverse);
    ...
    m_encoder = arm_motor.getEncoder(SparkRelativeEncoder.Type.kHallSensor, 42); // 42
    ...
    m_controller = arm_motor.getPIDController();
  ...
  public void runAutomatic() {
    ...
    m_controller.setReference(
        m_targetState.position, CANSparkFlex.ControlType.kPosition, 0, m_feedforward);
  }
  ...
  public void runManual(double _power) {
    ...
    arm_motor.set(_power + (m_feedforward / 12.0));
